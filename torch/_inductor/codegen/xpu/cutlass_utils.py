...
def try_import_cutlass() -> bool:
    ...
    try:
        ...
        log.debug(
            "Found cutlass_library in python search path, overriding config.sycl.cutlass_dir"
        )
        cutlass_library_dir = os.path.dirname(cutlass_library.__file__)
        assert os.path.isdir(
            cutlass_library_dir
        ), f"{cutlass_library_dir} is not a directory"
        config.sycl.cutlass_dir = os.path.abspath(
            os.path.join(
                cutlass_library_dir,
                "source",
            )
        ...
        log.debug(
            "cutlass_library not found in sys.path, trying to import from config.sycl.cutlass_dir"
        )
...
def _gen_ops_cached(arch, version) -> list[Any]:
    ...
    instantiation_level: str = config.sycl.cutlass_instantiation_level
    args = CUTLASSArgs(
        architectures=arch,
...
def sycl_standalone_runner_compile_command(srcpath: Path, exepath: Path):

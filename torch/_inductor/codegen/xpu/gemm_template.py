...
class CUTLASSGemmTemplate(CUTLASSTemplate, ABC):
    ...
    def header(self) -> IndentedBuffer:
        ...
        if inductor_sycl_config.generate_test_runner and not is_dynamic(
    ...
    def set_alignment(torch_layout, op_element) -> bool:
        ...
        cuda_arch = cutlass_utils.get_sycl_arch()
    ...
    def filter_op(
        ...
        if inductor_sycl_config.cutlass_op_allowlist_regex is not None:
            if not re.search(
                inductor_sycl_config.cutlass_op_allowlist_regex, op.configuration_name()
            ):
                return None
        if inductor_sycl_config.cutlass_op_denylist_regex is not None:
            if re.search(
                inductor_sycl_config.cutlass_op_denylist_regex, op.configuration_name()
            ):
                return None
    ...
    def gen_ops(self) -> "list[tuple[str, cutlass_gemm_op.GemmOperation]]":  # type: ignore[name-defined]  # noqa: F821
        ...
        return list(res.items())[: inductor_sycl_config.cutinductor_cuda_configlass_max_profiling_configs]
    ...
    def render(  # type: ignore[override]
        ...
        **kwargs,
        ...
        if inductor_sycl_config.generate_test_runner and not is_dynamic(X, W, Y, Bias):
...
class CUTLASS3xGemmTemplate(CUTLASSGemmTemplate):
    ...
    def render_gemm_arguments(
        ...
        Args:
            ...
            kernel (SYCLTemplateKernel): CUDA Template kernel for the operation.
